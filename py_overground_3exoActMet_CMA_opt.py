...
controlEffortWeight = 0; 
effortExponent = 2 
stateTrackingWeight = x[0] 
GRFTrackingWeight   = x[1] * 5e-8 *10 * 100 * 10 * 100

activationWeight = 8.0
activationWeightEach = 1e0

metabolicsWeight = 2.4e-4 * 15 * 0.5
metabolicsExponent = 2
...
trackedfile = './3CMA_exo_opt_2D3D_muscle_GaitTracking_tracked_states.sto'
resultspath = './results/2.7/' + str(finalTime*2)[2:] + '/'
...
track.setName('3CMA_exo_opt_2D3D_muscle_GaitTracking');
...
TreadmillModel = 'strong_mk14_rv1_dgf_kneedampremoved_exo.osim'
...
for m in range(numMuscles):
    ...
    musclemet.addMuscle(muscleName, osim.Muscle.safeDownCast(premetmodel.getComponent(musclePath)), ratio, 350000)
    premetmodel.addComponent(musclemet)
# premetmodel.addComponent(metabolics)
premetmodel.finalizeConnections()
premetmodel.printToXML('mk14_rv1_paths_dgf_met_exo.osim')
...
stateTrackingGoal.setWeightForState('/jointset/groundPelvis/pelvis_tilt/speed', 1000/((2.2*np.pi/180)**2));
stateTrackingGoal.setWeightForState('/jointset/groundPelvis/pelvis_list/value', 50/((2.6*np.pi/180)**2));
stateTrackingGoal.setWeightForState('/jointset/groundPelvis/pelvis_list/speed', 50/((2.6*np.pi/180)**2));
stateTrackingGoal.setWeightForState('/jointset/groundPelvis/pelvis_rotation/value', 50/((2.3*np.pi/180)**2));
stateTrackingGoal.setWeightForState('/jointset/groundPelvis/pelvis_rotation/speed', 50/((2.3*np.pi/180)**2));
...
# bounds of 1e7 to 1e10
stateTrackingGoal.setWeightForState('/jointset/groundPelvis/pelvis_ty/value', 2*tyweight);
stateTrackingGoal.setWeightForState('/jointset/groundPelvis/pelvis_ty/speed', 2*tyweight);
...
stateTrackingGoal.setWeightForState('/jointset/walker_knee_l/knee_angle_l/speed', 3* 5e3/((18.8*np.pi/180)**2))
stateTrackingGoal.setWeightForState('/jointset/walker_knee_r/knee_angle_r/value', 1e3/((18.8*np.pi/180)**2))
stateTrackingGoal.setWeightForState('/jointset/walker_knee_r/knee_angle_r/speed', 1e3/((18.8*np.pi/180)**2))

stateTrackingGoal.setWeightForState('/jointset/ankle_l/ankle_angle_l/value', 8e6/((12.9*np.pi/180)**2))
stateTrackingGoal.setWeightForState('/jointset/ankle_l/ankle_angle_l/speed', 0 * 8e6/((12.9*np.pi/180)**2))
stateTrackingGoal.setWeightForState('/jointset/ankle_r/ankle_angle_r/value', 4e4/((12.9*np.pi/180)**2))
stateTrackingGoal.setWeightForState('/jointset/ankle_r/ankle_angle_r/speed', 0 * 4e4/((12.9*np.pi/180)**2))
...
if 'metabolicsWeight' in locals():
    ...
    metabolicspsoas = osim.MocoOutputGoal('metabolicspsoas', metabolicsWeight)
...
if 'momenttrackweight' in locals():
    ...
    jointMomentTracking = osim.MocoGeneralizedForceTrackingGoal('joint_moment_tracking', momenttrackweight)
...
# add exotendon parameters to the moco problem.
if paramopt:
    HOBL = forceSet.get('HOBL')
    problem.addParameter(osim.MocoParameter('exotendon_stiffness', '/forceset/HOBL', 'stiffness', osim.MocoBounds(0, 1000)));
    problem.addParameter(osim.MocoParameter('exotendon_length', '/forceset/HOBL', 'resting_length', osim.MocoBounds(0, 0.5)));
    # mtpspringr = forceSet.get('mtp_spring_r')
    # problem.addParameter(osim.MocoParameter('mtp_spring_r_stiffness', '/forceset/mtp_spring_r', 'stiffness', osim.MocoBounds(0, 100)));
    # problem.addParameter(osim.MocoParameter('mtp_spring_r_rest_length', '/forceset/mtp_spring_r', 'rest_length', osim.MocoBounds(-1.04, 0.52)));   
    # problem.addParameter(osim.MocoParameter('mtp_spring_r_viscosity', '/forceset/mtp_spring_r', 'viscosity', osim.MocoBounds(0, 10)));
...
gaitTrackingSolution.write(resultspath + '3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution__Halfgaitcycle.sto');
# gaitTrackingSolution.write('./results/1_nat_muscles_Tracking_solution__Halfgaitcycle.sto');
# % keyboard
# gaitTrackingSolution = osim.MocoTrajectory(resultspath + '3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution__Halfgaitcycle.sto')
# testobj = 5
...
fullStride.write(resultspath + './3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution_FullStride.sto');
# write a controls file as well
osim.STOFileAdapter.write(fullStride.exportToControlsTable(), resultspath + '3CMA_exo_opt_controls.sto')
...
osim.STOFileAdapter.write(table_vel, resultspath + '3CMA_exo_opt_quickAnalysis_Normalized_Fiber_Velocity.sto')
...
osim.STOFileAdapter.write(table_len, resultspath + '3CMA_exo_opt_quickAnalysis_Normalized_Fiber_Length.sto')
...
osim.STOFileAdapter.write(table_probe, resultspath + '3CMA_exo_opt_quickAnalysis_metabolics.sto')
...
osim.STOFileAdapter.write(table_tenvel, resultspath + '3CMA_exo_opt_quickAnalysis_tendon_velocity.sto')
...
osim.STOFileAdapter.write(table_mtu, resultspath + '3CMA_exo_opt_quickAnalysis_mtu.sto')
...
osim.STOFileAdapter.write(table_pass, resultspath + '3CMA_exo_opt_quickAnalysis_passive_fiber_force.sto')
...
osim.STOFileAdapter.write(table_activ, resultspath + '3CMA_exo_opt_quickAnalysis_active_fiber_force.sto')
...
osim.STOFileAdapter.write(table_head.flatten(), resultspath + '3CMA_exo_opt_quickAnalysis_head_pos.sto') 
...
osim.STOFileAdapter.write(table_ToeL.flatten(), resultspath + '3CMA_exo_opt_quickAnalysis_bigToeL_pos.sto')
...
osim.STOFileAdapter.write(table_ToeR.flatten(), resultspath + '3CMA_exo_opt_quickAnalysis_bigToeR_pos.sto')
...
osim.STOFileAdapter.write(table_probeind, resultspath + '3CMA_exo_opt_quickAnalysis_metabolics_individuals.sto')
...
osim.STOFileAdapter.write(table_jointMoments, resultspath + '3CMA_exo_opt_quickAnalysis_joint_moments.sto');
...
osim.STOFileAdapter.write(table_exoten, resultspath + '3CMA_exo_opt_quickAnalysis_HOBL.sto')

analyzeStrings_mtu = osim.StdVectorString(); 
analyzeStrings_mtu.append('.*'); 
table_mtu = study.analyze(fullStride, analyzeStrings_mtu); 
osim.STOFileAdapter.write(table_mtu, resultspath + '3CMA_exo_opt_quickAnalysis_all.sto')
...
osim.STOFileAdapter.write(externalForcesTableFlat, 
                             resultspath + './3CMA_exo_opt_2D3D_OG_muscles_Tracking_solutionGRF_FullStride.sto');
fullstrideGRF = fullStride;
fullstrideGRF.insertStatesTrajectory(externalForcesTableFlat);
fullstrideGRF.write(resultspath + './3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution_fullStride_wGRF.sto');


print('\n\nchange wantguess to True if we want to overwrite the guess to this solution!')
print('gaitTrackingSolution to fullstrideGRF:  \n3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution_fullStride_wGRF.sto\n')
...
#                                             x, tag, modelmass, resultspath)
# generate a report
output = resultspath + '3CMA_exo_opt_report.pdf'
report = osim.report.Report(model,
                            resultspath + '3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution_fullStride_wGRF.sto',
                            bilateral=True,
                            output=output)
...
if comparisonreport:
    ...
    # out2 = resultspath + '3exoActMet_3exoActMet_compare_report.pdf'
    out2 = resultspath + '3CMA_exo_opt_Exotight_compare_report.pdf'
    # ref_files = [
    #         './results/2.7/648/3exoActMet_2D3D_OG_muscles_Tracking_solution_FullStride.sto',
    #         './results/2.7/648/3exoActMet_controls.sto']
    ref_files = [
            './goodresults/27ms_mk13_poly5/015_3exoActMet_648_extraTight/3exoActMet_2D3D_OG_muscles_Tracking_solution_FullStride.sto',
            './goodresults/27ms_mk13_poly5/015_3exoActMet_648_extraTight/3exoActMet_controls.sto']
    report = osim.report.Report(model,
                                    resultspath + '3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution_FullStride.sto',
...
# okay here is a report for most recent nat vs exo comparison
# comparisonreport = input("\n\nDo we want to run the comparison report? \nNote: need to have the other solutions written.\n\n 0 for no, 1 for yes.")
comparisonreport = '1'
if comparisonreport != '1':
    comparisonreport = 0
...
if comparisonreport:
    print(comparisonreport)
    # out2 = resultspath + '3exoActMet_3ActMet_compare_report.pdf'
    out2 = resultspath + '3CMA_NatVSExo_compare_report.pdf'
    # ref_files = [
    #         './results/2.7/6835/3ActMet_2D3D_OG_muscles_Tracking_solution_FullStride.sto',
    #         './results/2.7/6835/3ActMet_controls.sto']
    ref_files = [
            './results/2.7/6835/3CMA_nat_2D3D_OG_muscles_Tracking_solution_FullStride.sto',
            './results/2.7/6835/3CMA_nat_controls.sto']
    report = osim.report.Report(model,
                                    resultspath + '3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution_FullStride.sto',
                                    output=out2, bilateral=True,
                                    ref_files=ref_files)
...
if comparisonreport:
    print(comparisonreport)
    # out2 = resultspath + '3CMA_exo_opt_3exoActMet_compare_report.pdf'
    out2 = resultspath + '3CMA_exo_opt_ExoIK_compare_report.pdf'
    # ref_files = [
    #         './results/2.7/648/3exoActMet_2D3D_OG_muscles_Tracking_solution_FullStride.sto',
    #         './results/2.7/648/3exoActMet_controls.sto']
    ref_files = [
            './expData/2D2Darms/27_IK_exo_FullStride.sto',
            './expData/2D2Darms/27_IK_exo_controls.sto']
    report = osim.report.Report(model,
                                    resultspath + '3CMA_exo_opt_2D3D_OG_muscles_Tracking_solution_FullStride.sto',
                                    output=out2, bilateral=True,
                                    ref_files=ref_files)
